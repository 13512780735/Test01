 ...
 */
public class Coupon03Fragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener {
    private CouponListAdapter mCouponListAdapter;
    List<CouponModel.CouponBean> mCouponData = new ArrayList<>();
    private SwipeRefreshRecyclerView mRecycleView;
    private boolean isGetData = false;

    public Coupon03Fragment() {
                ...
                if ("1".equals(baseResponse.getStatus())) {
                    if (baseResponse.getData().getCoupon().size() > 0) {
                        for (int i = 0; i < baseResponse.getData().getCoupon().size(); i++) {
                            if (baseResponse.getData().getCoupon().get(i).getCtype().equals("2")) {
                                mCouponData.add(baseResponse.getData().getCoupon().get(i));
                            }
                            continue;
                        }
